...
**Branch**: `001-initial-shopping-list` | **Date**: 2026-08-29 | **Spec**: [spec.md](./spec.md)
...
**Target Platform**: One stateless Linux application container plus PostgreSQL; modern mobile and
desktop browsers; HTTPS in deployed environments
...
**Scale/Scope**: One household, normal members only, multiple lists, starter catalog and fixed
categories, custom products, recent needs, and no guest invitations or recurring suggestions
...
| Modular monolith | PASS | Modules are `identity`, `household`, `catalog`, `shopping`, and `collaboration`; tests verify boundaries. |
...
**Post-design re-check**: PASS. The data model and contracts preserve module boundaries,
PostgreSQL-only persistence, and the offline boundary. JobRunr is not introduced because this
release has no concrete persistent background-processing requirement.
...
**Structure Decision**: Use one Maven project with package-by-feature Spring Modulith modules.
Each module exposes a narrow API package; other modules cannot use its internals. Vaadin views
coordinate user interaction only and call application behavior in the owning module. The `shared`
package is limited to genuinely cross-cutting types.
